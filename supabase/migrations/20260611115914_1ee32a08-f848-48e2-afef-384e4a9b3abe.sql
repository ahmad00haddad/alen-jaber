...
DROP TRIGGER IF EXISTS trg_site_settings_upd ON public.site_settings;
...
CREATE TABLE IF NOT EXISTS public.projects (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  num text NOT NULL DEFAULT '', title text NOT NULL, tag text NOT NULL DEFAULT '',
  year text NOT NULL DEFAULT '', featured boolean NOT NULL DEFAULT false, link text,
...
CREATE TABLE IF NOT EXISTS public.stats (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  number text NOT NULL, label text NOT NULL, is_latin boolean NOT NULL DEFAULT false,
  sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.process_steps (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  num text NOT NULL DEFAULT '', title text NOT NULL, description text NOT NULL DEFAULT '',
  sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.marquee_words (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  word text NOT NULL, sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.nav_links (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  href text NOT NULL, label text NOT NULL, sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
DROP TRIGGER IF EXISTS trg_testimonials_upd ON public.testimonials;
DROP TRIGGER IF EXISTS trg_stats_upd ON public.stats;
DROP TRIGGER IF EXISTS trg_process_upd ON public.process_steps;
...
INSERT INTO public.site_settings (key, value) VALUES
  ...
  ('hero_meta', '{"corner_tl1":"© 2026","corner_tl2":"IRBID — JORDAN","corner_tr1":"REEL · 001","corner_tr2":"NOW SHOWING","image_index":"01 / 01","scroll_text":"SCROLL","strip_1":"FOX MULTIMEDIA","strip_2":"IRBID · JORDAN","strip_3":"EST. 2018"}'::jsonb),
  ...
  ('works_intro', '{"label":"— الأعمال / 02","headline":"أعمالٌ مختارة","year_range":"2023 — 2026"}'::jsonb),
  ...
  ('footer', '{"name":"ألِن جابر","description":"مخرج ومنتج محتوى إبداعي. أبني قصصاً بصرية للعلامات والأفلام من إربد، الأردن.","location":"إربد، الأردن","company":"FOX Multimedia Services","copyright":"© 2026 ALEN JABER — جميع الحقوق محفوظة","made_in":"MADE IN JORDAN","links_title":"روابط","services_title":"الخدمات","contact_title":"تواصل"}'::jsonb),
  ...
  ('nav_cta', '{"label":"متاح للعمل","href":"#contact"}'::jsonb)
...
DO $$ DECLARE t text; BEGIN
  FOR t IN SELECT unnest(ARRAY['site_settings','projects','testimonials','stats','process_steps','services','marquee_words','nav_links']) LOOP
